...
from gui.cursors import picking
...
def _is_alive(artist):
    ...
    return bool(artist
                ...
                and (artist.container in artist.axes.containers
                     if isinstance(artist, picking.ContainerArtist) else
...
class Cursor:
        ...
        if isinstance(aoc, Container):
            try:
                ca, = {artist for artist in (ref() for ref in self._artists)
                       if isinstance(artist, picking.ContainerArtist)
    ...
    def add_selection(self, pi):
        ...
        ann = axes.annotate(
            picking.get_ann_text(*pi), xy=pi.target,
    ...
    def _on_select_event(self, event):
        ...
        for artist in self.artists:
            if (artist.axes is None  # Removed or figure-level artist.
                ...
                continue
            pi = picking.compute_pick(artist, per_axes_event[artist.axes])
            if pi:
                pis.append(pi)
    ...
    def _on_key_press(self, event):
        ...
        for key in ["left", "right", "up", "down"]:
            if event.key == self.bindings[key]:
                self.remove_selection(sel)
                self.add_selection(picking.move(*sel, key=key))
                break
...
def cursor(pickables=None, **kwargs):
    ...
    **kwargs
    ...
    for container in containers:
        ...
        if contained:
            artists.append(picking.ContainerArtist(container))
