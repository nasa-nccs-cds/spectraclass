...
#        mesh: QuadMesh = self.image_template.plot.imshow( ax=self.plot_axes )
...
#                    self.clear_overlay_image( False )
    ...
    def onMouseClick(self, event):
        if event.xdata != None and event.ydata != None:
            ...
            t0 = time.time()
            if not self.toolbarMode and inaxes and (self.key_mode == None) and self.point_selection_enabled:
            ...
            lgm().log(f"Completed MouseClick event in {time.time()-t0} sec")
