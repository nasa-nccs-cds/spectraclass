...
Selection.artist.__doc__ = ( "The selected artist.")
Selection.target_.__doc__ = """
The point picked within the artist, in data coordinates.

:meta private:
"""
Selection.index.__doc__ = ( "The index of the selected point, within the artist data.")
Selection.dist.__doc__ = ( "The distance from the click to the target, in pixels.")
Selection.annotation.__doc__ = ( "The instantiated `matplotlib.text.Annotation`.")
Selection.extras.__doc__ = ( "An additional list of artists (e.g., highlighters) that will be cleared at the same time as the annotation.")
...
class _Target(np.ndarray):
    ...
    @property
    def index(self):
        warnings.warn( "Selection.target.index is deprecated and will be removed in the future; use Selection.index instead.")
        return self._sel.index

Selection.target = property( _Target, doc="The point picked within the artist, in data coordinates.")
