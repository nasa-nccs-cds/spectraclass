...
class AutoEncoderCluster(ClusterBase):
    ...
    def cluster_data(self) -> xa.DataArray:
        ...
        cdata = np.expand_dims( np.argmax(self._reduction * self.cscale, axis=1 ), 1 ) + 1
    ...
    def compute_reduction(self):
        ( self._reduction, reproduction, point_data ) = rm().reduce( self._input_data, self.method, self._n_clusters, self.nepochs)[0]
