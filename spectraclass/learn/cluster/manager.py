...
class ClusterManager(SCSingletonConfigurable):
    ...
    def generate_training_set( self, source: str, event ):
        ...
        for (image_index, block_coords, icluster, nclusters), marker in self._cluster_markers.items():
            ...
            lgm().log(f"#CM: Add {source} training chunk, input_data shape={input_data.shape}, xchunk shape={xchunk.shape}, "
                      f"ychunk shape={ychunk.shape}, #gids={marker.gids.size}, anomaly={input_data.attrs['anomaly']}")
            lgm().log(f" --> mask_array shape={mask_array.shape}, mask_array nzeros={np.count_nonzero(mask_array)}, "
                      f"ychunk shape={ychunk.shape}, #gids={marker.gids.size}")
