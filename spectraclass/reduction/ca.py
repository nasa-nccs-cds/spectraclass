...
class PCAReducer:
    ...
    def train( self, train_input: np.ndarray ):
        ...
        lgm().log(f"#PCA.train: reduced features shape={reduced.shape}, stat={stat(reduced)}")
        lgm().log(f"#PCA.train: reproduction shape={reproduction.shape}, stat={stat(reproduction)}")
    ...
    def get_component_graph( self, **kwargs ) -> hv.Overlay:
        ...
        popts = dict(width=600, height=300, yaxis="bare", ylim=(-1.0,1.0), alpha=0.6)
