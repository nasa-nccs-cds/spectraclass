...
print_rows = 25
n_neighbors = 4
dtypes = dict( samples=np.int32, model=np.int32, spectral_embedding=np.float32 )

device_data: cudf.DataFrame = read_csv( "spectraclass_data.csv", dtype=dtypes ).pivot(index='samples', columns='model')
