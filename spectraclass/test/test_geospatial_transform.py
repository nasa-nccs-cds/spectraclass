...
#        label_dataset: xa.Dataset = self.read_labels()
#        rio_label_dataset: RasterDataset = label_dataset.rio
        ( new_label_map, out_transform ) = mask( self._labels_map, [region], invert=True, nodata=iClass, filled=True )
        self._labels_map = self._labels_map.copy( data=new_label_map )
    ...
    def initLabels(self):
        ...
        nodata_value = -2
        labels_map: xa.DataArray = self.image_template.copy( data=np.full( self.image_template.shape, -1, np.int32 ) )
        labels_map.attrs['_FillValue'] = nodata_value
        labels_map.name = "labels"
        labels_map.attrs[ 'long_name' ] = [ "labels" ]
        labels_map["transform"] = Affine( *labels_map.transform[:6] )
        self.save_labels( labels_map.where( self.image_template.notnull(), nodata_value ) )
...
from spectraclass.data.base import DataManager, dm
dmi: DataManager = DataManager.initialize( "demo3",'aviris' )
