...
class cpActivationFlow(ActivationFlow):
    ...
    def setNodeData(self, nodes_data: xa.DataArray ):
        if self.reset or (self.nodes is None):
            if (nodes_data.size > 0):
                ...
                self.D: np.ndarray = self._knn_graph.neighbor_graph[1].astype(np.int32)
    ...
    def getGraph(self):
        if self._knn_graph is None:
            ...
            print( f"Computing NN-Graph with parms= {kwargs}" )
    ...
    def spread( self, sample_data: np.ndarray, nIter: int = 1, **kwargs ) -> Optional[bool]:
        ...
        print(f"Beginning graph flow iterations, #C = {label_count}, C[10] = {self.C[:10]}")
#        t0 = time.time()
        converged = False
        for iter in range(nIter):
            try:
#                print( f"iterate_spread_labels: " )
#                for iX, X in enumerate([ self.I, self.D, self.C, self.P ]): print( f" I{iX} -> {X.shape}:{X.dtype}" )
